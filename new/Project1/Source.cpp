...
float ClearColor[4] = { 0.0f, 1.0f, 0.5f, 1.0f }; // красный, зеленый, синий, альфа-канал
float DefaultColor[4] = { 0.0f, 1.0f, 0.5f, 1.0f }; // красный, зеленый, синий, альфа-канал
float ChangeRotation = 1;
bool SetColor = true;
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	wcex.lpszClassName = "Urok3WindowClass";
	...
	g_hWnd = CreateWindow("Urok3WindowClass", "Урок 3. Матрицы", WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	PAINTSTRUCT ps;
	HDC hdc;
	float ChangeColor[4] = { 1.0f,0.0f,0.51f,0.0f };
	...
	case WM_MOUSEMOVE:
		oss << "(" << MAKEPOINTS(lParam).x << "," << MAKEPOINTS(lParam).y << ")";
		SetWindowText(hWnd, oss.str().c_str());
		break;
	case WM_KEYDOWN:
		if (wParam == 'F')
		{
			SetWindowText(hWnd, "LAB01");
			
			if (SetColor)
			{
			for (int i = 0; i < 4; i++)ClearColor[i] = ChangeColor[i];
			SetColor = false;
			}
			else {
				for (int i = 0; i < 4; i++)ClearColor[i] = DefaultColor[i];
				SetColor = true;
			}
...
HRESULT InitGeometry()
{
	...
	hr = CompileShaderFromFile("urok3.fx", "VS", "vs_4_0", &pVSBlob);
	if (FAILED(hr))
	{
	...
	hr = CompileShaderFromFile("urok3.fx", "PS", "ps_4_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(NULL, "Невозможно скомпилировать файл FX. Пожалуйста, запустите данную программу из папки, содержащей файл FX.", "Ошибка", MB_OK);
		return hr;
	...
	{	/* координаты X, Y, Z				цвет R, G, B, A					 */
		...
		{ XMFLOAT3(1.0f,  0.0f, -1.0f), XMFLOAT4(0.0f, 1.0f, 1.0f, 0.0f) },
		{ XMFLOAT3(-1.0f,  0.0f,  1.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 0.0f) },
	...
	{	// индексы массива vertices[], по которым строятся треугольники
		...
		2,3,1,/* Треугольник 2 = vertices[0], vertices[3], vertices[4] */
...
void SetMatrixes()
{
	...
	g_World = XMMatrixRotationY(t*ChangeRotation);
